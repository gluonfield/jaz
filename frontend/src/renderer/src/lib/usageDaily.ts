import type { DailyUsage, Usage } from './api/types'
...
export function usageMonthLabels(cells: UsageCell[]): string[] {
  const weeks = cells.length ? Math.max(...cells.map((cell) => cell.week)) + 1 : 0
  const labels = Array.from({ length: weeks }, () => '')
  for (const cell of cells) {
    const date = parseLocalDate(cell.date)
    if (date.getDate() <= 7 && !labels[cell.week]) {
      labels[cell.week] = date.toLocaleDateString(undefined, { month: 'short' })
    }
  }
  return labels
}

export function sumUsage(days: DailyUsage[]): Usage {
  return days.reduce<Usage>((total, day) => {
    ...
    total.total_tokens = (total.total_tokens ?? 0) + (day.usage.total_tokens ?? totalTokens(day.usage))
    return total
  }, {})
}
...
export function peakDay(days: DailyUsage[]): DailyUsage | null {
  return days.reduce<DailyUsage | null>((peak, day) => {
    if (totalTokens(day.usage) === 0) return peak
    if (!peak || totalTokens(day.usage) > totalTokens(peak.usage)) return day
    return peak
  }, null)
}
