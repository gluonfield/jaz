...
import { BoardAssignmentPicker } from '@/components/loops/BoardAssignmentPicker'
...
import { deleteLoop, type LoopDetail, loopDetailQuery, runLoopNow, updateLoop } from '@/lib/api/loops'
...
function LoopDetail({
  ...
  return (
    <div className="mx-auto max-w-[820px] px-10 pb-20 pt-6">
      ...
      <WidgetBoards loop={loop} boardIds={boardIds} />
...
function WidgetBoards({ loop, boardIds }: { loop: Loop; boardIds: string[] }) {
  const boards = useQuery(boardsQuery)
  const queryClient = useQueryClient()
  const toast = useToast()

  const assign = useMutation({
    mutationFn: (nextBoardIds: string[]) => updateLoop(loop.id, { board_ids: nextBoardIds }),
    ...
    onError: (error) => toast(`Couldn't update boards: ${(error as Error).message}`, 'danger'),
  })

  const allBoards = boards.data ?? []
  const selected = assign.isPending && assign.variables ? assign.variables : boardIds
  const activeCount = selected.filter((id) => allBoards.some((board) => board.id === id)).length
