...
func TestStreamSessionEventsProjectsTextReplacementAcrossToolEvents(t *testing.T) {
	...
	textState := *acpState
	textState.TextRunID = "text-1"
	if err := store.AppendSessionEvents(session.ID,
		sessionevents.Event{Type: "acp_message", Content: "message chunks, t", ACP: &textState},
		sessionevents.Event{Type: "acp_tool", ACP: &toolState},
		sessionevents.Event{Type: "acp_message", Content: "ool calls", ACP: &textState},
	); err != nil {
		t.Fatal(err)
	}
	srv := &Server{Store: store, Events: sessionevents.New()}
...
func TestStreamSessionEventsKeepsProgressTextAcrossToolEvents(t *testing.T) {
	store, err := jsonstore.New(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	toolState := *acpState
	toolState.ToolCalls = []sessionevents.ACPToolCall{{ID: "tool-1", Title: "Read file", Status: "completed"}}
	firstTextState := *acpState
	firstTextState.TextRunID = "text-1"
	secondTextState := *acpState
	secondTextState.TextRunID = "text-2"
	if err := store.AppendSessionEvents(session.ID,
		sessionevents.Event{Type: "acp_message", Content: "I'll build a rich visual com", ACP: &firstTextState},
		sessionevents.Event{Type: "acp_tool", ACP: &toolState},
		sessionevents.Event{Type: "acp_message", Content: "Let me pull the chart guidance.", ACP: &secondTextState},
	); err != nil {
		t.Fatal(err)
	}
	srv := &Server{Store: store, Events: sessionevents.New()}
	...
	if !strings.Contains(body, "id: 3\n") || !strings.Contains(body, `"content":"Let me pull the chart guidance."`) {
		t.Fatalf("missing later progress text: %s", body)
	}
	if strings.Contains(body, `"replace_seqs"`) || strings.Contains(body, "I'll build a rich visual comLet me") {
		t.Fatalf("progress messages were projected as one text stream: %s", body)
	}
}
