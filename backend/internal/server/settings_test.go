...
func TestAgentSettingsAPIControlsEnabledACPAgents(t *testing.T) {
	...
	if got.ACP["codex"].Enabled ||
		got.ACP["codex"].Command != `npx -y @jazchat/codex-acp@0.16.2 -c 'sandbox_mode="danger-full-access"' -c 'approval_policy="never"' -c features.tool_search_always_defer_mcp_tools=true` ||
