...
func TestAgentSettingsAPIControlsEnabledACPAgents(t *testing.T) {
	...
	wantCodexCommand := `npx -y @jazchat/codex-acp@0.16.6 -c 'sandbox_mode="danger-full-access"' -c 'approval_policy="never"' -c features.tool_search_always_defer_mcp_tools=true`
	if runtime.GOOS == "windows" {
		wantCodexCommand = `npx.cmd -y @jazchat/codex-acp@0.16.6 -c 'sandbox_mode="danger-full-access"' -c 'approval_policy="never"' -c features.tool_search_always_defer_mcp_tools=true`
	}
