...
func TestManagerSteerCancelsQuestionCreatedAfterPromptQueued(t *testing.T) {
	store, err := jsonstore.New(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	spawned, err := manager.Spawn(ctx, acp.SpawnRequest{ACPAgent: "fake", Slug: "fake-question-after-queue"})
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _, _ = manager.Cancel(context.Background(), spawned.SessionID) }()

	if _, err := manager.Send(ctx, acp.SendRequest{Session: spawned.SessionID, Message: "queue before ask", Completion: acp.CompletionInline}); err != nil {
		t.Fatal(err)
	}
	if _, err := manager.Steer(ctx, acp.SteerRequest{Session: spawned.SessionID, Message: "say hello"}); err != nil {
		t.Fatal(err)
