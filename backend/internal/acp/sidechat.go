...
func (m *Manager) SendSideChat(ctx context.Context, req SideChatRequest) error {
	...
	m.publishSideChatUserMessage(job.eventSnapshot(), scope, message, contexts, req.Attachments)
	...
	if err != nil {
		m.publishSideChatMessage(job.eventSnapshot(), scope, "error", err.Error(), "error")
		return err
	}
...
func (m *Manager) applySideChatUpdate(job *jobState, update acpschema.DecodedSessionUpdate) bool {
	...
	case acpschema.AgentMessageChunkUpdate:
		...
		m.publishSideChatMessage(job.eventSnapshot(), scope, "assistant", contentText(event.Content), "running")
		return true
	case acpschema.AgentThoughtChunkUpdate:
		...
		m.publishSideChatMessage(job.eventSnapshot(), scope, "thought", contentText(event.Content), "running")
		return true
	case acpschema.ToolCallSessionUpdate:
		...
		m.publishSideChatMessage(job.eventSnapshot(), scope, "tool", firstNonEmpty(event.Title, string(event.ToolCallID)), status)
		return true
