...
func jobFromInactiveState(session storage.Session, state storage.ACPState) Job {
	...
	return Job{
		...
		Cwd:             state.Cwd,
		ModelProvider:   firstNonEmpty(state.ModelProvider, session.ModelProvider),
		Model:           firstNonEmpty(state.Model, session.Model),
		ReasoningEffort: firstNonEmpty(state.ReasoningEffort, session.ReasoningEffort),
