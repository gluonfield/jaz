...
func (t *Tool) Definition() tools.Definition {
	return tools.Function(
		"agent_spawn",
		"Create an idle ACP-backed agent session, such as codex or claude_code. This only creates the session; send tasks with agent_send and choose wait=true or wait=false per task. Pass directory to work inside an existing project; pass worktree=true to isolate repo changes on a session branch.",
		true,
