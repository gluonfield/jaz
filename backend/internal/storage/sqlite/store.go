...
func (s *Store) AppendMessages(id string, messages ...provider.Message) error {
	...
	existing, err := providerMessagesFromRecords(records)
	if err != nil {
		s.mu.Unlock()
		return err
	}
	all := append(existing, messages...)
	next, err := recordsFromProviderMessages(all, time.Now().UTC())
	if err != nil {
		s.mu.Unlock()
		return err
	}
	err = s.replaceMessagesLocked(id, next)
	s.mu.Unlock()
	if err != nil {
		return err
	}
	s.mirrorMessages(id, all)
	return nil
}
...
func (s *Store) resetStaleRunningThreads() error {
	_, err := s.db.Exec(`UPDATE threads SET status = ?, updated_at_ms = ? WHERE status = ?`,
		storage.StatusError, timeToMs(time.Now().UTC()), storage.StatusRunning)
	return err
}
...
func scanSession(scanner sessionScanner) (storage.Session, error) {
	...
	var title, parentID, acpAgent, acpSessionID, modelProvider, model, reasoningEffort sql.NullString
