...
const (
	...
	// identifies Jaz; Google access still requires a per-account refresh token.
...
func (c OAuthClientConfig) Credentials() (OAuthClientCredentials, error) {
	...
	if id == "" || secret == "" {
		return OAuthClientCredentials{}, errors.New("both Google OAuth client ID and secret are required when overriding the bundled client")
	}
